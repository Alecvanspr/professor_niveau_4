...
export function AdminFrameworkPage() {
  ...
  async function seedProfessorFramework() {
    const response = await fetch(`${API_BASE}/frameworks/seed/professor`, {
      method: 'POST',
    })
    const payload = await response.json()
    if (!response.ok) {
      setMessage(payload.detail ?? 'Seeden mislukt')
      return
    }
    setMessage(`Professor niveaukader beschikbaar met id ${payload.id}`)
  }
  ...
  return (
    <div>
      ...
      <p>Of laad het volledige aangeleverde professor-niveaukader uit config.</p>
      <button onClick={seedProfessorFramework}>Laad professor niveaukader</button>
      {message && <p>{message}</p>}
    </div>
  )
}
